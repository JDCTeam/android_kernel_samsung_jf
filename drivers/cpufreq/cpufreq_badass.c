...
static inline cputime64_t private_get_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_bds_info_s *bds_info;
		bds_info = &per_cpu(od_cpu_bds_info, j);
		bds_info->prev_cpu_idle = private_get_cpu_idle_time(j,
...
static void bds_check_cpu(struct cpu_bds_info_s *this_bds_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = private_get_cpu_idle_time(j, &cur_wall_time);
...
static void bds_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < policy->max) {
		...
		__cpufreq_driver_target(policy, policy->max,
					CPUFREQ_RELATION_L);
		this_bds_info->prev_cpu_idle = private_get_cpu_idle_time(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_bds_info->prev_cpu_idle = private_get_cpu_idle_time(j,
						&j_bds_info->prev_cpu_wall);
			if (bds_tuners_ins.ignore_nice) {
				j_bds_info->prev_cpu_nice =
